...
## Druhé kolo auditu (2026-04-28 odpoledne)

Po dokončení 8 vln byl proveden druhý hloubkový průchod hledající, co první audit nepokryl. Zjištěny a opraveny tyto problémy:

### R2-1 Error stránky (commit `fabf8ec`)

**Nález:** `templates/bundles/TwigBundle/Exception/error{,403,404,500}.html.twig` používaly legacy Bootstrap třídy (`.container`, `.row`, `.col-md-8`, `.text-center`) bez příslušného CSS. Texty se vykreslovaly flush s viewport okraji, bez vrchního paddingu, vizuálně mimo design system.

**Fix:** Nový partial `_partials/error.html.twig` se sjednocenou kostrou (eyebrow / code / title / description / action). 4 šablony Exception/ přepsány na include partialu. CSS `.error-shell` / `.error-card` / `.error-eyebrow` / `.error-code` (clamp(72px, 12vw, 128px)) / `.error-title` / `.error-description` / `.error-actions` v `chrome.css`. Layout vertikálně centrovaný v dostupném viewportu, max-width 60ch. Přepsané texty jsou kratší, přímější ("Tato stránka tu není" místo "Stránka nenalezena. Omlouváme se, ale...").

### R2-2 Em-dash v Chapters katalogu (commit `fabf8ec`)

**Nález:** Titul kapitoly 12 v `src/Catalog/Chapters.php` byl `'DDD v praxi — kde to bolí'` s em pomlčkou. Footer (renderovaný z katalogu) zobrazoval em pomlčku napříč všemi stránkami.

**Fix:** Sjednoceno na en pomlčku v jediném zdroji.

### R2-3 Meta_description nad 160 znaků (commit `a1f0f79`)

**Nález:** Dva templaty mírně překračovaly SERP snippet práh: `hub_practice` (161 znaků) a `anti_patterns` (164 znaků).

**Fix:** Přeformulováno bez ztráty informace na 151 a 148 znaků. Ostatních 23 šablon je v pásmu 100–160 znaků.

### R2-4 ASCII pomlčky přes řádky (commit `63bf7a7`)

**Nález:** Předchozí prózový sweep (vlna 1, T-P1-2) hledal pouze ` - ` (space-hyphen-space) v jediném řádku. V 10 souborech zbylo 51 výskytů, kde ASCII pomlčka stála na konci řádku a pokračování přes whitespace + odsazení na následující řádek (HTML to renderuje jako ` - `, ale původní regex tu variantu nezachytil).

**Fix:** Nový regex `(?<=[^\s<\-]) -[ \t]+(?=\S)` ošetřuje obě varianty. Distribuce: glossary 12, ddd_ai 10, ddd_pain_points 10, sagas 7, cqrs 4, when_not_to_use_ddd 3, event_sourcing 2, anti_patterns 1, practical_examples 1, implementation_in_symfony 1. Po opravě 0 ASCII prose pomlček zbývá ve všech 17 šablonách.

### R2-5 Sitemap + dateModified bump (commit `7532404`)

**Nález:** Po dnešních úpravách (PLACEHOLDER recovery, ASCII pomlčky, scroll-shadow, error pages, atd.) zůstaly `article_modified_time` bloky a `<lastmod>` sitemapy na předchozích datech.

**Fix:** Všech 18 článkových šablon a 25 sitemap entries bumpnuto na 2026-04-28.

### Verifikace funkčnosti (bez nálezu)

Tyto interakce byly v R2 testovány a fungují korektně:

- **Skip-link** – `#content` cíl, focus po tabu na první.
- **TOC scroll-spy** – IntersectionObserver aktivuje `.toc-current` v desktop sidebaru i mobilní `<details>` paralelně.
- **Mobile drawer** – `role="dialog"`, `aria-modal="true"`, focus se přenese na první link po otevření, ESC zavře a vrátí focus na toggle.
- **Copy button** – přidá `.code-copied` třídu, label se mění na „zkopírováno ✓", po 1.5 s vrátí.
- **FAQ accordion** – `<details>` toggle funguje, schema.org/FAQPage JSON-LD validní.
- **Heading hierarchy** – jediná `h1` per stránka, `h2` sekce, `h3` subsekce uvnitř `<main>`.
- **Internal anchors** – 0 broken `href="#..."` linků na cqrs (validováno přes querySelectorAll).
- **Sitemap = templates dates** – po R2-5 sjednoceno (script ověřil 0 mismatches).
- **Heading order** – kontrolováno na cqrs (62 nadpisů), žádný skip levelu.
- **Console errors** – jediný error na 404/403 stránkách je očekávaný HTTP status code, ne chyba.
