...
Důraz je na správnou aplikaci strategických i taktických vzorů DDD v reálném projektu.
...
První workshop byl orientován na otázku „co se v systému děje". Doménoví experti formulovali v chronologickém
...
Slovník událostí odhalil několik důležitých rozhodnutí ještě před prvním řádkem kódu. Slovo „uživatel" má
...
Ubiquitous Language vznikl ve spolupráci s doménovými experty ještě před zahájením implementace. Stejné pojmy se používají v kódu, dokumentaci i komunikaci. Klíčové pojmy:
...
není stejný uživatel jako autor akce, je toto zpoždění akceptovatelné. Pro real-time notifikace by tento
kompromis nestačil – tam je třeba synchronní integrace nebo websocket push z projekce.
...
bylo by třeba vrátit stav přes kompenzační scénář – vzor, který popisuje kapitola
...
bylo by třeba buď přesunout pravidlo do doménové služby s jasným kontraktem, nebo z `Task`
udělat komponentu uvnitř `Project` agregátu (málo škálovatelné, ale konzistentní s ohledem
na invariant). Detailní rozbor transakčních hranic je v kapitole
...
3. **Agregáty a hranice transakcí** – Správné definování agregátů a jejich hranic umožnilo konzistentní správu dat. Každý agregát byl zodpovědný za udržování své vnitřní konzistence a byl měněn v jedné transakci.
4. **Doménové události pro integraci** – Doménové události zajistily komunikaci mezi různými bounded contexts. Například, když byl vytvořen nový úkol, byla vygenerována událost `TaskCreated`, která mohla být zpracována jinými kontexty bez vytváření přímých závislostí.
...
10. **Trade-offy je třeba dokumentovat, ne řešit** – Ne každé rozhodnutí má jednu správnou odpověď. Sdílený kernel pro identifikátory, eventual consistency u auditu, synchronní ACL přes port – každá z těchto voleb má cenu, kterou tým akceptoval s vědomím alternativy. Záznam těchto rozhodnutí v dokumentaci (ADR) zachoval kontext pro pozdější refaktor; bez něj by se za půl roku diskuse opakovala znovu.
...
- question: Co bylo nejtěžším rozhodnutím projektu?
  answer: 'Volba mezi synchronním ověřením členství v projektu (přes port <code>ProjectChecker</code>) a asynchronní reakcí přes lokální projekci. Synchronní cesta je v monolitu jednodušší, ale vytváří časovou závislost mezi kontexty. Tým zvolil synchronní variantu jako pragmatický kompromis pro fázi monolitu, s vědomím, že při budoucím štěpení do služeb bude třeba refaktor na lokální projekci. Plný kontext rozhodnutí včetně dalších čtyř trade-offů v <a href="#trade-offs">sekci Výzvy a rozhodnutí</a>.'
:::
