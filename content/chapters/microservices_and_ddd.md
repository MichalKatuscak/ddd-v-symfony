...
[Bounded Context](/zakladni-koncepty#bounded-contexts) je ohraničená oblast s jedním konzistentním doménovým modelem a jedním Ubiquitous Language. [Context Mapping](/context-mapping) popsal, jak spolu kontexty komunikují, a [ságy a Process Managery](/sagy-a-process-managery) přidaly koordinaci procesu napříč nimi: kompenzace místo distribuovaných transakcí.

Tato kapitola odpovídá na otázku, kterou si dříve nebo později položí každý tým: **jak se z logických hranic stanou fyzické nasazovací jednotky?** Konkrétně: kdy se Bounded Context mapuje na microservice a kdy ne. Kapitola hájí tři teze, které se často přehlížejí. Mapování 1:1 (BC = service) je jen jedna ze tří možností. Pro většinu týmů je *modular monolith* rozumnější výchozí bod. A špatně navržené microservices jsou horší než monolit, kterému se snaží uniknout.

## 19.01 Mýtus „microservice = Bounded Context“ {#mytus}

V komunitě DDD a microservices koluje v různých variantách slogan: *„Each microservice should be one Bounded Context.“* Kořen má v článku Jamese Lewise a Martina Fowlera *Microservices* (2014), kde se mluví o *přirozené korelaci* mezi hranicí služby a hranicí kontextu. Korelace se cestou ztvrdla v rovnici: DDD vymezuje hranici modelu, microservices hranici nasazení, tak proč obojí neztotožnit. Praxe to nepotvrzuje. Slogan je **polopravda**, která vede k chybným architektonickým rozhodnutím častěji než ke správným.
...
Sam Newman v knize *Building Microservices, 2nd ed.* (2021) tuto distinkci zdůrazňuje opakovaně. V kapitole 2 píše, že Bounded Context je silným kandidátem na service boundary. Rozhodnutí, zda kontext skutečně dostane vlastní nasazovací jednotku, závisí na faktorech jako velikost týmu, rozdílné potřeby škálování, různý release cyklus a operační kapacita organizace. Chris Richardson v knize *Microservices Patterns* (2018) v kapitole 2 popisuje stejné rozhodnutí jako dekompozici podle business capabilities a podle subdomén a zdůrazňuje, že rozdělení musí mít doménový důvod, ne čistě technický.
...
Tabulka shrnuje, čím se Bounded Context a microservice liší a v jaké rovině se o nich rozhoduje. Většina anti-vzorů v této kapitole vzniká z toho, že tým jednu úroveň plete s druhou.
...
Mapování 1:1 mezi Bounded Contextem a microservice se v komunitě podává jako výchozí cíl. Automatické pravidlo to ale není: platí jen tam, kde tým splní konkrétní organizační a technické předpoklady. Sam Newman je v knize *Building Microservices, 2nd ed.* shrnuje pod hlavičkami „information hiding“ a „autonomy“. Service má smysl, když ji lze měnit, nasazovat a škálovat nezávisle na zbytku systému. Pojem *information hiding*, převzatý od Davida Parnase, vykládá hned v kapitole 1 prostě: skrýt uvnitř komponenty co nejvíc, vystavit navenek co nejméně. Na hledání hranic ho aplikuje v kapitole 2.
...
- **Rozdílný compliance režim** – kontext zpracovává citlivá data (PCI DSS, zvláštní kategorie osobních údajů podle GDPR), která vyžadují striktní oddělení od ostatního systému. Network isolation a samostatný audit trail jsou přirozenějším řešením, když kontext žije ve vlastní service.

### Příklad: e-shop se čtyřmi services {#priklad-eshop-heading}

Středně velká e-commerce platforma s 30 inženýry ve čtyřech stream-aligned týmech identifikovala na workshopu [Event Storming](/event-storming) čtyři Bounded Contexty:
...
Podstatné slovo je **obhajitelné**, ne „správné“. Microservices jsou rozhodnutí s kompromisy: vyšší operační složitost, potřeba distributed tracingu, eventual consistency všude, kde dříve stačila ACID transakce. Kompromisy rozebírají další sekce.

### Jak velká má být jedna service {#velikost-service-heading}

Použitelnou odpověď dává Zhamak Dehghani v článku *How to break a Monolith into Microservices* (2018) heslem **„macro first, then micro“**: začít u větších služeb postavených kolem jednoho doménového konceptu a dělit je dál, až je na to tým provozně připravený. Na pořadí záleží. Sloučit dvě předimenzované služby je refaktoring; rozpletení sítě dvaceti nano-services je projekt na čtvrtletí.
...
Při **třech a méně** zůstaňte v [modular monolithu](#modular-monolith). Rozdělení by stálo víc, než ušetří.
:::
...
- **Nestabilní hranice** – produkt je v rané fázi a Bounded Contexty ještě procházejí iteracemi. Refaktor hranice uvnitř monolitu je triviální (přesun souborů a tříd); refaktor přes síťovou hranici dvou services je migrace dat, koordinovaný release a Anti-Corruption Layer.
- **Podobné potřeby škálování všech kontextů** – pokud catalog, ordering i shipping mají podobný objem a profil, není co odděleně škálovat. Horizontální škálování celého monolitu je operačně levnější než škálování čtyř services.
- **Nemáte operační platformu pro N services** – žádný Kubernetes, žádný service mesh, žádné centralizované logging a tracing. Bez nich microservices technicky poběží, ale ladění incidentů bude pomalé a drahé. Více v [sekci o provozu](#ops).
...
V Symfony se modular monolith přirozeně staví strukturou adresářů pod `src/`. Každý Bounded Context dostane vlastní namespace a podadresář, uvnitř typicky se třemi vrstvami (Domain, Application, Infrastructure). Aplikace se tak dělí nejdřív svisle podle kontextů a teprve uvnitř na vrstvy; srovnání se [svislým řezem](/architektonicke-styly#vertical-slice) po funkcích je v kapitole o architektonických stylech:
...
Všimněte si dvou detailů. `SharedKernel` drží **malou sdílenou podmnožinu doménového modelu**, tedy hodnotové objekty jako `Money` a `Currency`, na jejichž výkladu se všechny kontexty shodly. Kniha do něj dává jen hodnotové objekty. Evans obsah kernelu neomezuje, sdílené entity nebo události ale zvětšují plochu, na které se musí týmy shodnout. Změna Shared Kernelu vyžaduje souhlas všech vlastníků, a protože je to drahé, musí zůstat malý. Celý vzor rozebírá [Context Mapping](/context-mapping#shared-kernel). Druhý detail: každý BC má vlastní `Application/IntegrationEvent/`, kam mapuje příchozí události z jiných kontextů. Stejný princip platí v sekci 19.08 i mezi samostatnými services.
...
return static function (Config $config): void {
    ...
    foreach ($contexts as $context) {
        ...
        // Pravidlo 3: doménové události zůstávají uvnitř svého kontextu.
...
S těmito pravidly lze v monolitu zůstat měsíce i roky. Závislosti v kódu mezi BC hlídá CI podobně tvrdě jako síťová hranice, za řádově nižší operační složitost. Sdílené tabulky ale phparkitect nevidí; vlastnictví dat musí hlídat oddělená schémata nebo konvence.
...
Postup migrace rozebírá [sekce 19.09 (Strangler Fig)](#migrace).

## 19.04 Distributed Monolith – anti-vzor {#distributed-monolith}

Distributed monolith má **vnější tvar microservices** (N samostatných services, N deploymentů, N repozitářů, N týmů), ale **vnitřní coupling monolithu** (sdílená databáze, synchronní volání všude, coordinated release, sdílená library s doménovými typy). Platíte všechny náklady microservices bez jejich výhod. Dražší způsob, jak provozovat monolit, není.

Vzniká dvěma cestami a výsledek bývá stejný. **První cesta**: tým rozdělil monolit do services dříve, než identifikoval Bounded Contexty. Hranice mezi services jsou náhodné (podle technické vrstvy nebo CRUD entit), ne podle domény, takže services spolu musí komunikovat o všem. **Druhá cesta**: tým rozdělil správně podle BC, ale neověřil, že každá service má skutečnou autonomii. Sdílená databáze „pro jednoduchost“, sdílená library s doménovými typy „aby se neopakoval kód“, sdílená deployment pipeline „aby release byl atomický“.
...
5. **Sdílená library s doménovými typy.** Existuje balíček `company/domain-shared`, který obsahuje třídy jako `OrderPlaced`, `Money`, `CustomerId` používané všemi services. Změna v balíčku vynucuje současný release všech services. Coupling je tu silný stejně jako v monolitu, jen se schovává za package version.
:::

### Proč je horší než monolith {#proc-distributed-monolith-heading}

Coupling monolitu s operační režií microservices spojuje to nejhorší z obou světů:

- **Latence.** Vnitřní volání monolitu je volání funkce (~µs); volání mezi services je síťová cesta tam a zpět (~ms) plus serializace, deserializace a validace. Rozdíl tří řádů na každé volání. Při 10 vnořených voláních narostou mikrosekundy na desítky milisekund.
...
- **Refaktoring.** Přesunutí pole z jedné entity do jiné je v monolitu refaktoring v IDE. Mezi services je to migrace dat, změna API kontraktů, koordinovaný deploy a období dual-write.
- **Testovací prostředí.** Místo `composer install && vendor/bin/phpunit` potřebujete docker-compose s deseti kontejnery a 32 GB RAM.

Detailní rozbor obecných anti-vzorů, které k distributed monolithu vedou (microservices first, shared DB, sync everywhere), najdete v kapitole [Anti-vzory DDD](/anti-vzory).
...
- **Kontexty v jádru jsou úzce provázané** (Catalog ↔ Ordering ↔ Customer)
...
- **Tým neunese N+1 services** – core monolit vlastní jeden tým, každý extrakt
  přidá operační dluh.
...
Nejcitovanějším příkladem je **Amazon Prime Video** (článek z března 2023, pozornost
vzbudil v květnu). Kolem něj se ale nabalila historka, která s původním textem souhlasí
jen zčásti:
...
- **Latence tvořená hlavně síťovými skoky** mezi services. Taková interakce má být
  volání funkce v jednom procesu.
...
3. **Replikace doménového kódu.** Service A se stane modulem `App\Catalog\` v monolitu.
...
Záleží i na pojmenování: **de-microservicing je legitimní architektonická volba**, ne selhání.
Reaguje na změnu kontextu (tým se zmenšil, profil zátěže se vyrovnal,
operační kapacita klesla). Komunikujte ho jako „consolidation“, ne jako „regression“.
:::
...
Se dvěma services přichází rozhodnutí, jak spolu komunikují. Nabízejí se dva základní interakční vzory. **Synchronní** jde přes REST, gRPC nebo SOAP; **asynchronní** přes message broker (RabbitMQ, Kafka, NATS, AWS SNS/SQS). Většina reálných systémů kombinuje obojí. Volba určuje výslednou provázanost, latenci a dostupnost.
...
- **Cross-BC reakce, kde jednotlivé BC nemají závislost na výsledku.** Sága zpracovává krok po kroku přes eventy + commands; každý krok je nezávislý.
...
- Asynchronní messaging má lepší časové oddělení: subscriber může být dočasně nedostupný a publisher to neví. Při sync volání je volající přímo závislý na uptime volaného.
...
- Asynchronní tok přirozeněji zapadá do modelu z [Event Stormingu](/event-storming), který stojí právě na doménových událostech.
...
Osa sync/async je jen polovina rozhodnutí. Druhá polovina zní: co se do události vejde. Martin Fowler v článku *What do you mean by „Event-Driven“?* (2017) rozlišuje čtyři vzory, které se pod tímto pojmem skrývají. Pro integraci služeb jsou podstatné dva a v praxi se pletou.
...
Ukázka v sekci 19.08 patří do druhé kategorie. `OrderPlacedReceived` nese `orderId`, `customerId`, částku i měnu, takže `billing-svc` vystaví fakturu bez jediného dotazu zpět do `ordering-svc`. Mezi oběma režimy se volí vědomě a pro jeden směr integrace se drží jeden z nich. Míchání obou v jednom toku končí tím, že nikdo neví, která data jsou autoritativní.
...
Cena vzoru se ukáže, jakmile jsou potřeba data ze dvou services najednou. `JOIN` přes hranici neexistuje a nahrazují ho tři možnosti:
...
## 19.06 Distribuované transakce – sága, ne 2PC {#distribuovane-transakce}

Jakmile doménový proces překročí hranici jednoho Bounded Contextu (a v microservices architektuře tedy hranici jedné service), vzniká otázka **konzistence napříč services**. ACID transakce, na kterou je tým zvyklý v jedné databázi, v distribuovaném prostředí neplatí. Klasickou odpovědí kdysi býval *Two-Phase Commit* (2PC, XA transactions). V microservices architektuře je 2PC **prakticky nepoužitelný**.
...
### Sága jako odpověď {#saga-heading}
...
Sága existuje ve dvou variantách:
...
Detailní implementaci ság v Symfony 8 (kompenzace, idempotence, choreografie vs. orchestrace, timeouty, paralelní kroky) probírá samostatná kapitola [Ságy a Process Managery](/sagy-a-process-managery). Pro účely této kapitoly stačí vědět dvě věci. Sága je **v DDD kontextu doporučovaný mechanismus pro distribuované transakce v microservices**. Daní za to je eventual consistency a kompenzační logika.

:::callout{type="note"}
### Sága vs. 2PC – shrnutí {#saga-vs-2pc-heading}

2PC se snaží zachovat ACID model přes hranici sítě a v praxi to končí blokádou nebo in-doubt stavem. Sága ACID opouští. Akceptuje, že systém je dočasně nekonzistentní a že se konzistence obnoví přes sémantické kompenzace. Pro doménové experty je to často přirozenější model než 2PC. Doménový proces v reálném světě (objednávka, platba, expedice) obvykle běží jako sekvence kroků s explicitní undo strategií, ne jako jeden atomický commit.
:::

## 19.07 Service mesh, observability, provoz {#ops}

Microservices jsou především **operační problém**, ne programátorský. Tým, který přejde z monolitu na deset services, najednou řeší věci, které dříve obstarával operační systém a Symfony framework: routing mezi procesy, retry, circuit breaking, mTLS, distribuovaný debug, service discovery, centralizované logy, rate limiting. Každá z těchto věcí má svůj nástroj a cenu a dohromady tvoří stack, který někdo musí provozovat.

Minimum pojmenoval Martin Fowler už v roce 2014 v textu *MicroservicePrerequisites*: rychlé provisionování prostředí, základní monitoring a rychlý deployment aplikace, k tomu organizační posun směrem k DevOps. Kdo tyto schopnosti nemá, má si je podle Fowlera vybudovat dřív, než pustí microservices do produkce. Zbytek sekce Fowlerovo minimum rozepisuje do dnešních nástrojů.

### Service mesh {#service-mesh-heading}

Service mesh (Istio, Linkerd, Consul Connect) je infrastrukturní vrstva, která řeší cross-cutting concerns mezi services:

- **mTLS** – vzájemná autentizace přes TLS bez nutnosti manuálního managementu certifikátů.
- **Retry a circuit breaking** – automatické opakování a otevření okruhu při opakovaných selháních.
...
**Kdy service mesh:** 10+ services, multi-team organizace, požadavek na mTLS bez ruční práce, potřeba pokročilého traffic managementu (canary, blue/green s percentage routing). **Kdy ne:** 3–5 services, malý tým, žádný Kubernetes. Provozní režie převáží přínosy.

### Observability – three pillars {#observability-heading}

V monolitu stačila kombinace strukturovaných logů a metrik. V microservices přibývá třetí pilíř, distributed tracing. Centralizovaně se pak musí řešit všechny tři:

- **Logs** – centralizovaná agregace logů (ELK / Loki / CloudWatch). Každý řádek logu musí nést `trace_id` a `service_name`, jinak časovou řadu událostí napříč services nesložíte.
- **Metrics** – Prometheus + Grafana, nebo cloudový ekvivalent (Datadog, New Relic). Standardní metriky (RED – rate, errors, duration) per service a per endpoint.
- **Traces** – OpenTelemetry + Jaeger / Tempo / Honeycomb. Jeden user request se trasuje napříč všemi services, každý skok má span. Bez toho je ladění prakticky nemožné: pět services a dvacet logů v incidentu nedá dohromady jednu časovou řadu.
...
Na čtvrtý příznak distributed monolithu ze sekce 19.04, tedy že end-to-end test vyžaduje všechny services, existuje kanonická odpověď: **consumer-driven contracts**. Konzument zapíše, co od poskytovatele očekává (jaká pole, jaké typy, jaké stavové kódy), a z toho vznikne kontrakt. Poskytovatel ho pak ověřuje ve své vlastní pipeline, bez běžícího konzumenta. Testovací sada přestane potřebovat runtime všech N services a rozpadne se na N nezávislých sad.

Nejznámější nástroj je Pact. Jeho podpora v PHP je slabší než v Javě nebo JavaScriptu, takže napojení stojí víc práce než v jazycích, ze kterých vzor pochází. Alternativa dostupná hned: udržovat schéma integračních událostí jako verzovaný artefakt a v CI kontrolovat, že payload z publishera projde deserializací u každého subscribera. Pokrytí je užší, ale zachytí přesně tu chybu, která v provozu bolí nejvíc. Obecnou strategii testování rozebírá [kapitola o testování](/testovani-ddd).

:::callout{type="warn"}
### Operační pravidlo {#ops-pravidlo-heading}

Pokud nemáte **všechno** z tohoto seznamu (orchestrátor, centralizované logging, distributed tracing, service discovery, CI/CD per service), je modular monolith rozumnější volba. Bez observability jsou z microservices jen oddělené Symfony aplikace, které se nekoordinovaně volají.

Přechod z modular monolithu na microservices je primárně **investice do operační platformy**, ne do architektonického refaktoru. Smysl má, až když tým má kapacitu platformu budovat, nebo rozpočet na managed služby (EKS / GKE / AWS Fargate / Datadog).
:::

## 19.08 Symfony konkrétně – kdy a jak {#symfony}

Symfony 8 obslouží obě architektury, modular monolith i microservices, bez zásadní změny kódu v doménové vrstvě. Rozdíl je v **konfiguraci routingu Messengeru**: uvnitř monolitu všechny eventy a commands směřujete na `sync` transport (přímé volání) nebo na lokální `async` transport (doctrine/redis) s workerem; přes hranici dvou services je směrujete na `amqp` transport, který fyzicky publikuje zprávu do RabbitMQ.

### Modular monolith v Symfony {#symfony-monolith-heading}

V monolitu jsou všechny BC ve stejném Symfony procesu. Cross-BC integrace probíhá přes Domain Events + Symfony Event Dispatcher (jeden DI kontejner, přímý handler) nebo přes Symfony Messenger se `sync` transportem (in-process command bus pattern). Publikující BC doménovou událost na hranici přeloží na integrační událost s primitivy. Handler v druhém BC ji přijme, namapuje na **vlastní DTO** a spustí lokální command. Přímý odběr cizí doménové události by porušil pravidlo 3 z [phparkitect konfigurace](#phparkitect-rules-heading). Hranice je čistě v kódu, vynucená phparkitectem.

Nejlevněji se cross-BC integrace v monolitu udělá takto. Application Layer publikujícího BC vystaví rozhraní (port), které implementuje konzument. Žádné HTTP, žádný broker. Pokud později rozdělíte BC do services, port zůstane stejný, jen se za ním objeví HTTP klient nebo Messenger.

### Microservice v Symfony {#symfony-microservice-heading}

V microservice architektuře je každý BC vlastní Symfony aplikace s vlastním `composer.json`, `config/`, `src/`, vlastní DB. Integrace reagující na změny stavu probíhá **asynchronně** (synchronní dotazy a validace ze sekce 19.05 stranou) přes Symfony Messenger s AMQP transportem (RabbitMQ) v kombinaci s [Outbox patternem](/outbox-pattern) v publisheru a *Inbox idempotency* v subscriberovi.

Pravidlo: **publisher a subscriber *nesdílejí* PHP třídu eventu.** Publisher má svou doménovou událost v `App\Ordering\Domain\Event\OrderPlaced` uvnitř ordering-svc kódu. Subscriber v billing-svc má vlastní `App\Billing\Application\IntegrationEvent\OrderPlacedReceived`, samostatnou třídu, která se naplní z deserializovaného AMQP payloadu. Důvody jsou v sekci 19.04 (sdílená library s doménovými typy = distributed monolith).

:::callout{type="pattern"}
### YAML: Messenger config – publisher (ordering-svc) {#messenger-publisher-heading}

:::code{language="yaml" filename="ordering-svc/config/packages/messenger.yaml" highlights="49,50,51,52,53"}
...
framework:
    messenger:
        ...
        routing:
            # CRITICAL: routujeme VLASTNÍ doménovou třídu, ne sdílenou.
            # Na drát ale nejde její tvar: OutboundEventSerializer výše je
            # překladová vrstva a posílá integrační payload s primitivy.
...
`IntegrationEventSerializer` je vlastní serializer, který namapuje AMQP payload (typicky JSON) na konkrétní třídu integration eventu. Subscriber se rozhoduje podle řetězce `event_type` v hlavičce, ne podle PHP typu, a tím se od publishera *plně oddělí na úrovni kódu*.
...
Serializer je jediné místo v subscriberu, které zná formát publishera. Změny v doménové události publishera zasáhnou jen sem, zbytek kódu pracuje s vlastní třídou.

Dvojice „vlastní DTO plus překladová vrstva na hranici“ je [Anti-Corruption Layer](/context-mapping#acl) z Context Mappingu, jen realizovaný přes serializer místo přes klientskou třídu. Ostatní vztahy z Context Map mají v microservices architektuře stejně přímočarý protějšek:
...
:::code{language="php" filename="billing-svc/src/Infrastructure/Messaging/IntegrationEventSerializer.php" highlights="27,28,29,30"}
<?php
...
final readonly class IntegrationEventSerializer implements SerializerInterface
{
     ...
     * Od Symfony 8.1 s ní receiver pošle zprávu běžnou retry/failure
     * cestou do failure transportu. Na 8.0 ji AmqpReceiver jen rejectne,
     * takže přežije pouze v dead-letter exchange nastavené na frontě.
     * Jiná výjimka shodí worker, zpráva zůstane neackovaná a po restartu
     * se vrátí znovu – nekonečná smyčka nad jedinou vadnou zprávou.
     */
...
Za deserializací stojí už standardní Messenger handler. Převede příchozí integration event na lokální command vlastního BC:
...
Aplikace je *shared-nothing*: bootstrapuje se při každém requestu a mezi requesty si nedrží ani in-process cache, ani connection pool. Každá extrahovaná služba tedy platí vlastní bootstrap na vlastním requestu. Samo o sobě je to drobnost, jenže se násobí počtem skoků v řetězci.
...
- **Worker režimy.** FrankenPHP, RoadRunner nebo Swoole drží aplikaci v paměti mezi requesty. Bootstrap odpadá a cena synchronního volání klesá. Daň: služby s request-scoped stavem musí implementovat `Symfony\Contracts\Service\ResetInterface`, jinak stav prosákne do dalšího requestu.
...
Plný výklad vzoru je v kapitole [Migrace z CRUD na DDD](/migrace-z-crud#strangler-fig): princip koexistence staré a nové části, struktura projektu i srovnání s přímým přepisem. Tato sekce se soustředí na to, co je specifické pro extrakci do samostatných services: pořadí fází a kritéria, kdy zastavit.
...
- Postavte nový Symfony projekt jako samostatnou service. Zkopírujte (nikdy ne `git mv`) kód cílového BC z monolitu.
- Migrace dat: tabulky cílového BC postupně replikujte do nové DB. Nastane období dual-write, kdy píší monolit i nová service. Čtecí provoz přepínejte postupně. Dual-write je zde vědomá výjimka: [kapitola o Outboxu](/outbox-pattern#dual-write) ho popisuje jako problém, protože zápis do dvou úložišť není atomický. Při migraci je únosný jako řízený a časově omezený stav, s jedním úložištěm jako zdrojem pravdy a s pravidelnou rekonciliací, která rozdíly najde a srovná. Trvalé řešení z něj nedělejte.
- Cross-BC eventy převeďte na AMQP transport Messengeru. Subscriber má vlastní integration event DTO (sekce 19.08).
- Po stabilizaci smažte zbytky cílového BC z monolitu.

#### Fáze 3: Iterace nebo zastavení {#faze-3-heading}

**Cíl:** rozhodnout, zda pokračovat dalším BC, nebo zastavit a žít s hybridní architekturou (monolit + 1–2 services). Hybrid je **legitimní cíl**, ne dočasná fáze. Mnoho úspěšných systémů k čisté microservices architektuře nikdy nedospěje, protože k tomu nemá důvod.
...
Přepis na zelené louce s plánovaným přepnutím „za 18 měsíců“ končí zpravidla nedokončeným novým systémem vedle stárnoucího monolitu. Typický průběh tohoto selhání a důvody, proč k němu dochází, rozebírá [Migrace z CRUD na DDD](/migrace-z-crud#big-bang-warning-heading).
:::

## 19.10 Anti-vzory v microservices a DDD {#antivzory}

Pět nejčastějších anti-vzorů při kombinaci DDD a microservices, každý s konkrétním symptomem a opravou.
...
**Důsledek:** jakákoli změna schématu zlomí všechny services, které tabulku konzumují. Žádná service data nevlastní. Každý refaktoring databáze znamená koordinovanou migraci všech konzumentů.

**Oprava:** data dělit podle BC. Cross-BC čtení nahradit voláním API (sync) nebo replikací přes eventy (async, eventually consistent). Žádný cross-BC join na DB úrovni. Kanonickou podobu anti-vzoru s ukázkami kódu popisují [Anti-vzory](/anti-vzory#sdilena-databaze).
...
Obecnější rozbor anti-vzorů v DDD (nejen microservices) najdete v kapitole [Anti-vzory](/anti-vzory).

## 19.11 Shrnutí {#summary}

Bounded Context je **logická hranice modelu**, microservice **fyzická hranice deploymentu**. Mapování mezi nimi je 1:1, 1:N nebo N:1 a slogan „BC = microservice“ slouží jen jako výchozí hypotéza.

Kapitola nese jediné rozhodnutí: kudy vede hranice mezi procesy.
Pro tým do třiceti lidí ji nejlevněji uhlídá modular monolith. Závislosti mezi BC
v kódu vynutí phparkitect za zlomek operační složitosti. Samostatnou service si BC zaslouží,
když splní aspoň čtyři podmínky ze sekce 19.02: vlastní stream-aligned tým, vlastní data,
nezávislý release cyklus, jiné potřeby škálování, odlišný stack, compliance isolation.
Tři a méně znamená zůstat v monolitu.

Když už hranice vede přes síť, drží ji tři pravidla. Data mají jednoho vlastníka:
jedno schéma se dvěma zapisovateli je porucha, dotaz přes hranici řeší
...
Symfony Messenger obslouží oba světy: sync transport uvnitř monolitu, AMQP transport
...
s doménovými typy patří k příznakům distributed monolithu ze sekce 19.04, stejně jako sdílená
databáze a synchronní volání všude. A distributed monolith je horší než monolit, se kterým
jste začínali.
...
discovery a CI/CD per service, je odpovědí na otázku po hranici stejně modular monolith.
...
- [Chris Richardson – *Microservices Patterns* (Manning, 2018)](https://microservices.io/book). Praktická kniha plná konkrétních patternů. Kapitola 2 (dekompozice podle business capabilities a subdomén), kapitola 3 (interprocess communication), kapitola 4 (sagas), kapitola 13 (refaktoring monolitu).
...
- question: Můžu mít 2 Bounded Contexty v jedné microservice?
  answer: 'Ano, a často je to správné rozhodnutí. Je to definice <strong>modular monolithu</strong> nebo malého „mikro-monolithu“. Pokud dva BC sdílejí stream-aligned tým a podobné potřeby škálování, jejich provozování ve dvou samostatných services je operační overhead bez benefitu. Hlavní podmínka: hranice mezi BC <em>uvnitř</em> service musí být vynucená kódem (typicky phparkitect pravidly). Pokud se hranice obcházejí, máte nestrukturovaný monolit, ne modular monolith. Detail v <a href="#modular-monolith">sekci 19.03</a>.'
- question: Kdy přejít z monolitu na microservices?
...
- question: Co je BFF (Backend For Frontend) a kam patří v DDD?
  answer: 'BFF je vzor, ve kterém má každý frontend (web, mobile, partner API) <em>vlastní</em> backend agregátor, který volá downstream microservices a sestavuje view-model přesně přizpůsobený danému klientovi. V DDD terminologii je BFF downstream konzument volaných služeb: využívá jejich <a href="/context-mapping#ohs">Open Host Service</a> a <a href="/context-mapping#published-language">Published Language</a> a proti jejich modelům staví Anti-Corruption Layer. Sám OHS není. OHS je protokol otevřený všem konzumentům, kdežto BFF slouží jedinému klientovi. Doménovou logiku nenese; je to integrační vrstva s vlastním modelem obrazovek, kterou lze vést jako samostatný kontext (typicky „Web Frontend“).'
- question: GraphQL Federation jako náhrada microservices integrace?
  answer: 'GraphQL Federation umožňuje, aby více services vystavilo svou část schématu a aby gateway (Apollo Router) je z pohledu klienta sloučila do jednoho schématu. Pro <em>read</em> operace přes microservices odstíní klienta od fyzického rozdělení. Pro <em>write</em> operace federation neřeší distribuované transakce; pořád potřebujete <a href="/sagy-a-process-managery">ságu</a>. Doporučení: federation jako read fasáda, nikoli jako náhrada eventem řízené architektury.'
- question: Která service vlastní data o zákazníkovi napříč BC?
