...
většinou neupozorňuje – míst, kde se architektonické principy střetávají s realitou frameworku,
...
[Outbox](/event-sourcing#outbox) + [Sagu](/sagy-a-process-managery)),
zavolejte explicitně
`beginTransaction()` / `commit()` v Application Service
a oba repozitáře volejte v téže transakci. Toto je **přijatelná výjimka
z pravidla jeden agregát = jedna transakce** za předpokladu, že oba agregáty
...
Bohaté agregáty (metody jako `totalPrice()`, `items()`)
mohou neúmyslně triggerovat lazy load *mimo otevřenou transakci* nebo
*po detach()*, což vyústí ve výjimku
...
(`SEQUENCE`, `AUTO_INCREMENT`). To znamená, že nově vytvořený
...
a obě zprávu zpracovat. Navíc pořadí SELECT + zpracování + INSERT znamená,
že pokud handler vyhodí výjimku, klíč se nezapíše a zpráva se zkusí znovu –
což je správné chování, ale odhaluje jiný problém: pokud INSERT provedeme
...
Obě události `OrderPlaced` a `OrderShipped` jsou odeslány
za sebou, ale `OrderShipped` zpracuje jiný worker rychleji –
...
- question: Proč tradiční Doctrine mapování komplikuje čistý doménový model?
  answer: 'Doctrine očekává klasické PHP třídy s veřejnými nebo reflektovanými atributy, zatímco DDD agregát vyžaduje neměnnost, privátní settery a invarianty vynucené v konstruktoru. Konflikt zahrnuje identifikaci přes generované ID (Doctrine) oproti identitě v doméně (DDD), problém „špinavého" EntityManageru při dlouhých transakcích a omezení typů pro hodnotové objekty. Řešením je XML mapování držené mimo doménovou třídu a selektivní použití embeddables. Podrobný rozbor v <a href="#doctrine">sekci Doctrine vs. doménový model</a>.'
