...
povyšuje na architektonický vzor – formálně popsaný Gregem Youngem
...
CQRS je často používán v kombinaci s [Event Sourcing](/event-sourcing),
což je vzor, který ukládá změny stavu jako sekvenci událostí místo ukládání aktuálního stavu.
...
## 13.02 CQS vs. CQRS – kde je hranice? {#cqs-vs-cqrs}
...
CQRS není binární volba. Existuje spektrum úrovní zavedení, od nejjednodušší po nejpokročilejší:

1. **Oddělené handlery** – Command handlers a query handlers jako samostatné třídy, ale sdílí tutéž databázi a ORM entity. Nejjednodušší forma CQRS, vhodná pro většinu aplikací.
2. **Oddělené modely** – Write model používá doménové entity (Doctrine ORM), read model používá vlastní DTO/ViewModely plněné přímým SQL nebo Doctrine DBAL. Sdílená databáze, ale oddělené PHP třídy.
...
CQRS se v DDD typicky aplikuje **per Bounded Context**,
...
- **Oddělení odpovědností** – Write model se soustředí výhradně na doménovou logiku,
  validaci invariantů a konzistenci dat. Read model se soustředí na rychlou prezentaci dat
  uživateli. Každý model obsahuje přesně to, co potřebuje – nic navíc.
...
## 13.04 Výzvy a omezení CQRS {#challenges}

CQRS není stříbrná kulka. Jako každý architektonický vzor přináší kompromisy,
...
- **Eventual consistency** – Při asynchronní propagaci změn z write strany na read stranu
  ...
  formuláře vidět „starou" verzi dat. Řešení tohoto problému vyžaduje promyšlený přístup
  v UI – viz [sekce Eventual Consistency](#eventual-consistency).
...
- Vyvíjíte jednoduchou aplikaci s minimální doménovou logikou – klasický CRUD
  s Doctrine ORM bude jednodušší a rychlejší na vývoj.
...
## 13.05 Symfony Messenger jako základ CQRS {#symfony-messenger}
...
## 13.06 Implementace Commands {#commands}
...
- Pracuje typicky s primitivními typy (string, int, float) nebo serializovatelnými hodnotovými objekty (např. `OrderId`, `Money`); podstatné je, aby bylo možné command bezpečně přenést přes asynchronní kanál.
...
## 13.07 Implementace Queries {#queries}
...
## 13.09 ViewModely a Read Modely {#view-models}
...
## 13.10 Implementace Command a Query Buses {#buses}
...
Základní infrastruktura CQRS – příkazy, dotazy, handlery a busy – je tímto pokryta.
...
## 13.11 Optimalizace Read Modelů {#read-model-optimalizace}

Jednou z hlavních výhod CQRS je možnost **nezávisle optimalizovat read stranu**
pro konkrétní dotazy. Zatímco write model je normalizovaný (aby chránil konzistenci dat),
read model může být maximálně denormalizovaný – přesně ve tvaru, který potřebuje UI.
...
Nejrozšířenější strategií v praxi je vytvoření **denormalizované tabulky**,
...
Klíčovou výhodou CQRS s asynchronními projekcemi je možnost **kompletního rebuildu
read modelu**. Pokud se změní struktura denormalizované tabulky (nový sloupec,
jiný formát dat), stačí:
...
## 13.12 Eventual Consistency v praxi {#eventual-consistency}
...
## 13.13 Asynchronní zpracování {#async}

Jednou z výhod CQRS je možnost asynchronního zpracování příkazů. V Symfony 8 se asynchronní
...
- **Graceful shutdown** – Při deployi je třeba workerům poslat signál
...
## 13.14 Zpracování chyb a Dead Letter Queue {#error-handling}
...
Po vyčerpání všech pokusů o retry je zpráva přesunuta na **failed transport**
...
## 13.15 Middleware v CQRS {#middleware}

Symfony Messenger middleware je řetěz (chain) komponent, které zpracovávají zprávu
před a po jejím předání handleru. Middleware umožňuje přidat cross-cutting concerns
(validace, logování, transakce, autorizace) bez změny samotných handlerů.
...
## 13.16 Testování CQRS {#testovani-cqrs}

CQRS přirozeně vede k **snadno testovatelné architektuře**. Command handlers,
query handlers a projektory jsou izolované komponenty s jasně definovanými vstupy
a výstupy. Testovací strategie se liší podle testovaného komponentu:
...
## 13.17 Saga / Process Manager {#saga}
