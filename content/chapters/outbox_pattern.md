...
- **Většina moderních brokerů XA nepodporuje.** RabbitMQ má částečnou
...
nebo *dispatcher*) pak asynchronně tabulku polluje, vybírá řádky se stavem
...
(broker poslal ACK, event je trvale uložen), ale relay crashne *před* tím,
...
řádků a 100 `pending` to znamená, že se každých 100 ms scanuje milion
...
Klient pošle v hlavičce `Idempotency-Key` UUID, server si první request uloží
do cache (Redis nebo DB tabulka `http_idempotency`) spolu s odpovědí, a všechny
...
Defaultní bus chování v Symfony 8 je *auto-commit per dispatch*, ne per
handler – snadný zdroj chyby.
:::
...
Outbox Pattern má **přímočarou implementaci a velký provozní přínos**: vyřeší
