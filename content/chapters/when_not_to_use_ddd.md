...
## 23.02 1. CRUD admin a jednoduchý backoffice {#crud-admin}
...
## 23.03 2. Startup – doména se mění každý sprint {#startup}
...
[Event Storming](/event-storming) a [Context Mapping](/context-mapping) – mohou být
...
## 23.04 3. Malý tým bez doménových expertů {#small-team}
...
- DDD zaveďte až ve chvíli, kdy máte přístup k doménovým expertům a čas na [Event Storming](/event-storming).
:::

## 23.05 4. Data pipeline, ETL a reportovací systémy {#data-pipeline}
...
- **Symfony Messenger** pro asynchronní zpracování pipeline kroků – bez DDD overhead. Viz [kapitola o CQRS](/cqrs) pro inspiraci, jak Messenger používat v praxi.
:::

## 23.06 5. Projekt s životností kratší než rok {#short-lived}
...
- Pokud projekt nečekaně vyroste, refaktorovat z jednoduchého kódu na DDD je snazší než vysvětlovat, proč jednoduchý projekt má 40 tříd. Postup najdete v [Migraci z CRUD](/migrace-z-crud).
:::

## 23.07 6. Tým DDD nezná a čas na učení není {#no-knowledge}
...
## 23.08 7. Doména je nejasná, experti nejsou k dispozici {#unclear-domain}
...
- **Event Storming napřed** – než napíšete první řádek kódu, zmapujte doménu se stakeholdery. Bez toho neví DDD co modelovat. Více o Event Stormingu v kapitole [Event Storming](/event-storming).
